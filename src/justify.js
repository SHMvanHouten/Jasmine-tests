...
Justify.prototype.getJustifiedText = function(text, length){
    ...
    for(var i = 0; i< fittingSentencesArrayLength-1; i += 1){
        ...
        if (lengthDifference === 0){justifiedText += fittingSentencesArray[i].join("")}
...
var justify = function(str, len) {
    ...
    this.getJustifiedText = function(text, length){
        var wordsArray = text.split(" ");
        var fittingSentencesArray = [wordsArray[0]];
        var lineLength = wordsArray[0].length;
        var i = 1;
        ...
        for(i; i<wordsArray.length; i += 1){
            ...
            if(lineLength <= length){
                ...
                if(i === wordsArray.length -1){
                    justifiedText += fittingSentencesArray.join("")
                }
            }else{
                var fittingSentencesArrayJoined = fittingSentencesArray.join("");
                var lengthDifference = length - fittingSentencesArrayJoined.length;
                if(lengthDifference === 0){
                    justifiedText += fittingSentencesArrayJoined;
                }else{
                justifiedText += this.addSpaces(fittingSentencesArray, lengthDifference);
                }
                justifiedText += "\n";

                lineLength = wordsArray[i].length;
                if(lineLength >= length){justifiedText += wordsArray[i]}
                else{fittingSentencesArray = [wordsArray[i]];}
            }
        }
        return justifiedText;
    };
    return this.getJustifiedText(str, len);
};
