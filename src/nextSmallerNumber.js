...
function NextBiggerNumberFinder(){
    this.outputString = "";
    this.ascertainNextBiggerNumber = function(arr){
        arr.reverse();
        for (var i = 0; i<arr.length; i++){
            for (var j = 0; j<i; j++){
                if(arr[j]>arr[i]){
                    var tempArr = arr.splice(j,1);
                    var tempArr2 = arr.splice(0,i).sort();
                    tempArr = tempArr.concat(tempArr2);
                    return this.outputString += arr.reverse().concat(tempArr).join("");
                }
            }
...
NextBiggerNumberFinder.prototype.getNextBiggerNumber = function(number){
    var numberArray = number.toString().split("");
    this.ascertainNextBiggerNumber(numberArray);
    if(!this.outputString){return -1}
    return Number(this.outputString);
};
